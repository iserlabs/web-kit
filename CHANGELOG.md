...
## Unreleased
- **Audits (browser tier):** `web-kit audit --tier browser` — every route at thirteen real geometries in headless Chrome, checking what a page DOES rather than what its markup says: does the hero fill the window to the fold, does the page scroll sideways, is any text clipped, is anything past the right edge, is every tap target 44px, are there console errors, and (on phone geometries) does a real swipe still move the page. Zero new dependencies: it drives the system Chrome over CDP with Node's built-in WebSocket, so the sites whose whole dependency list is five packages can run it. Configured by `browser: { heroSelector, heroRoutes, routes, minTapTarget }` in `web-kit.audits.config.mjs`; `heroSelector` is the only opt-in, everything else applies to every site. Read-only by design: it never clicks, because a blind click sweep on a marketing site submits forms and sends enquiries. Findings are `error` (correctness blocks the ship) with per-check severity overrides as usual.
